...
oc new-app --image-stream="svcrouter-test2/testapi:v1-$version" --name testapi-v-1 APPVERSION=v1
sleep 8
echo "Create new app all in one with internal error has 20%" 
oc new-app --image-stream="svcrouter-test2/testapi:v1-$version" --name testapi-v-2 APPVERSION=v2 ERRORCODE=500
sleep 8
echo "Create new app all in one with timeout app 5s has 10%" 
oc new-app --image-stream="svcrouter-test2/testapi:v1-$version" --name testapi-v-3 APPVERSION=v3 TIMEWAITSEC=5
